...
function convert(keyArr, valueArr){
    ...
    let booksObj ={};                           // Tips: Step 1 - Create a local scoped object literal.
    for (let i =0; i < keyArr.length; i++) {    // Step 2: Define a for-loop that run based on keyArr's length.
        booksObj[keyArr[i]] = valueArr[i];      // Step 3: Within the for-loop, add the key and value to the local scoped object.
    }
